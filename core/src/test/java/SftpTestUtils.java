			...
			template.execute(new SessionCallback<LsEntry, Void>() {
				...
				public Void doInSession(Session<LsEntry> session) throws IOException {
					...
					for (int i = 0; i < fileNames.length; i++) {
						stream.reset();
						session.write(stream, "Consumer/" + fileNames[i]);
					}
					return null;
				}
			});
		...
		if (template != null) {
			template.execute(new SessionCallback<LsEntry, Void>() {
				...
				public Void doInSession(Session<LsEntry> session) throws IOException {
					...
					for (int i = 0; i < fileNames.length; i++) {
						try {
							session.remove("Consumer/" + fileNames[i]);
						}
						catch (IOException e) {}
					}
					try {
						// should be empty
						channel.rmdir("Consumer");
					}
		...
		if (template != null) {
			return template.execute(new SessionCallback<LsEntry, Boolean>() {
				...
				public Boolean doInSession(Session<LsEntry> session) throws IOException {
					...
					for (int i = 0; i < fileNames.length; i++) {
						try {
							SftpATTRS stat = channel.stat("Consumer/" + fileNames[i]);
							if (stat == null) {
